...
def train(model, data_loader, dataset, config):
    ...
    for key, value in model.named_parameters():
        # print(key)
        if 'fc3' in key or 'mlp' in key or 'att' in key or 'ffm' in key or 'csr' in key or 'imm' in key or 'sfm' in key or 'cls' in key:
            print(key)
    ...
    for epoch in range(start, config.epochs+1):
        ...
        for mode in ['train', 'val', 'test']:
            ...
            for i, [data, label] in enumerate(data_loader[mode]):
                ...
                out = F.sigmoid(out)
                out_disc = out
                loss_disc = criterion_disc(out_disc, label['disc'].float())
                loss = loss_disc.sum()
